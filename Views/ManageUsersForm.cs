            ...
            if (createEditUserForm.ShowDialog() == DialogResult.OK)
            {
                ...
                if (AuthenticationService.AddUser(createdUser.Username, createdUser.PasswordHash, createdUser.Role))
                {
                    ...
                    if (changePasswordForm.ShowDialog() != DialogResult.OK)
                    {
                        ...
                        DatabaseHelper.LogAudit(_authService.CurrentUser.Username, EventMapping.EventType.UserCreationAborted, $"Creation of user {createdUser} aborted.");
        ...
        private void ButtonDeleteUser_Click(object? sender, EventArgs e)
        {
            ...
            if (confirmResult == DialogResult.Yes)
            {
                if (AuthenticationService.RemoveUser(username))
                {
                    ...
                    DatabaseHelper.LogAudit(_authService.CurrentUser.Username, EventMapping.EventType.UserDeleted, $"Account {username} has been deleted");
                    LoadUsers();
